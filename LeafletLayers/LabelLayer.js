 ...
 * @property {number[]|L.LatLng} latLng Position of a label. If label is created using {@link L.ALS.LeafletLayers.LabelLayer#addLabel}, this property will be an array. Though, you can safely assign `L.LatLng` to it, it's always good to be consistent with the coordinates and use an array.
 */
...
L.ALS.LeafletLayers.LabelLayer = L.ALS.LeafletLayers.CanvasLayer.extend( /** @lends L.ALS.LeafletLayers.LabelLayer.prototype */{
	 ...
	 * @param latLng {number[]|L.LatLng} Position of a label
	...
	addLabel: function (id, latLng, text, displayOptions = this._defaultLabelOptions) {
		...
		L.ALS.LeafletLayers.LabelLayer._setLabelLatLng(label, latLng);
	...
	setLabelLatLng: function (id, latLng) {
		L.ALS.LeafletLayers.LabelLayer._setLabelLatLng(this._labels[id], latLng);
		if (this._automaticallyRedraw)
			this.redraw();
	},
	...
	statics: {
		...
		/**
		 * Sets label position
		 * @param label {LabelProperties} Label to set position of
		 * @param latLng {number[]|L.LatLng} Position to set
		 * @private
		 */
		_setLabelLatLng: function (label, latLng) {
			let lat, lng;
